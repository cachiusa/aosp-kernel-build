...
function menuconfig() {
  ...
  if [ -z "${FRAGMENT_CONFIG}" ]; then
    ...
    echo "Updating ${KERNEL_DIR}/arch/${ARCH}/configs/${DEFCONFIG}"
  ...
  KCONFIG_CONFIG=${new_fragment} ${KERNEL_DIR}/scripts/kconfig/merge_config.sh -m ${FRAGMENT_CONFIG} ${changed_config}
